...
class LearntOutput : CaDiCaL::Learner
{
    CaDiCaL::Solver *solver;
    std::vector<int> clause;
    bool output_clauses;
    FILE* learnt_clauses_file;

public:
    unsigned clauses;
    LearntOutput(CaDiCaL::Solver *s, const char* path, bool output) : solver(s), clauses(0), output_clauses(output)
    {
        if (output_clauses) {
...
#endif
#ifdef LEARNT_CLAUSES
        "  -learnt-file <file>   where to write the learnt clauses learnt-clauses.csv by default\n" 
#endif
        "\n"
...
int App::main (int argc, char **argv) {
  ...
  init ();
//   LearntOutput wrap (solver, stdout);
  ...
  const char *learnt_file = 0;
  bool proof_specified = false, dimacs_specified = false;
  bool output_clauses = false;
...
#endif
#ifdef LEARNT_CLAUSES
    else if (!strcmp (argv[i], "--output-learnts")) {
      output_clauses = true;
    }
    else if (!strcmp (argv[i], "--learnt-file")) {
      if (++i == argc)
        APPERR ("argument to '--learnt-file' missing");
      else if (learnt_file)
        APPERR ("multiple learnt files '--learnt-file %s' and '--learnt-file %s'",
                learnt_file, argv[i]);
      else {
        learnt_file = argv[i];
      }
  ...
  if (!learnt_file) {
    learnt_file = (std::string("learnts") + std::to_string(timeSinceEpochMillisec()) + std::string(".txt")).c_str();
  }
  LearntOutput wrap (solver,
    learnt_file,
    output_clauses);
